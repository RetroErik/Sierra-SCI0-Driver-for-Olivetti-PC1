; PC1-4.DRV - An SCI video driver for Olivetti Prodest PC1
;
; ARCHITECTURE:
;   Rectangle-based update. Two-pass bank-separated optimization attempt.
;   Pass 1: copy all even rows in rectangle (VRAM bank 0)
;   Pass 2: copy all odd rows in rectangle (VRAM bank 1)
;   Eliminates per-row interlace toggle overhead (PC1-2 has 4-instruction cost/row).
;
; PERFORMANCE THEORY:
;   Should save 4 instructions/row × 100 rows = 400 instruction savings.
;   Direct VRAM writes, no buffering overhead.
;
; VISUAL RESULT:
;   ✗ FAILED: Creates visible interlacing artifacts on hardware.
;   Even rows updated first, then odd rows → temporal aliasing/flicker.
;   Screen shows clear "combing" effect during updates.
;   CGA interlace pattern REQUIRES interleaved writes to maintain quality.
;
; LESSON:
;   Per-row toggle in PC1-2 is NOT wasted overhead — it's architectural necessity.
;   Optimization must respect hardware constraints (CGA interlace semantics).
;   Trading visual quality for 2-3% speed is a losing proposition.
;
; STATUS: ✓ Tested on hardware — produces visual artifacts. Educational failure case.
;
; Based on PC1-2.ASM (rectangle update, V40 optimized inner loop)
...
; - Rectangle-aware with two-pass even/odd bank copy
; - WARNING: Produces temporal aliasing due to sequential bank writes
...
;
; TWO-PASS APPROACH (PC1-4 optimization):
;   Pass 1: copy all even rows in the rectangle (VRAM bank 0, offset 0)
;   Pass 2: copy all odd rows in the rectangle  (VRAM bank 1, offset 8192)
;   Each pass writes sequentially within one VRAM bank — no toggling.
;   Source advances by 320 bytes per output row (skip the alternate row).
;   Eliminates the compare/branch interlace toggle of PC1-2.
;
...
.x_loop:
	...
	and     ax,0xF0F0       ; keep high nibble of each
	shr     ah,4            ; move pix2 to low nibble (V40/186)
	or      al,ah           ; [pix0 | pix2]
	stosb
	loop    .x_loop
	...
	pop     di
	pop     si
	add     si,320          ; skip 2 source rows (320 = 2 × 160)
	add     di,80           ; next row in same bank (sequential)
